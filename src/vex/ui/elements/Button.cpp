...
void Button::update() {
    float width = p_size.x / p_parent->getSize().x;
    float height = p_size.y / p_parent->getSize().y;

    float x = p_position.x;
    float y = p_position.y;

    float xOffset = 1 - ((x / p_parent->getSize().x) * 2) - (width);
    float yOffset = 1 - ((y / p_parent->getSize().y) * 2) - (height);
